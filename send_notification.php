...
$data_qry="SELECT * FROM tbl_diets ORDER BY `id` DESC";
$data_result=mysqli_query($mysqli,$data_qry); 

function get_single_info($post_id,$param,$type='diet')
{
  global $mysqli;

  switch ($type) {
    case 'diet':
    $query="SELECT * FROM tbl_diets WHERE `id`='$post_id'";
    break;
...
if(isset($_POST['submit']))
{
  ...
  if($_POST['diet_id']!=0){
    $id=$_POST['diet_id'];
    $type='diet';
    $title=get_single_info($id, 'diet_title','diet');
  }
                ...
                <form action="" name="addeditcategory" method="post" class="form form-horizontal" enctype="multipart/form-data">
                  <div class="section">
                    ...
                    <div class="col-md-9 mrg_bottom link_block">
                      <div class="form-group">
                       <label class="col-md-4 control-label">Diets :-<br/>(Optional)
                         <p class="control-label-help">To directly open single diet when click on notification</p></label>
                         <div class="col-md-8">
                          <select name="diet_id" id="diet_id" class="select2" required>
                            <option value="0">--Select Diets--</option>
                            <?php
                            while($data_row=mysqli_fetch_array($data_result))
                            {
                              ?>                       
                              <option value="<?php echo $data_row['id'];?>"><?php echo $data_row['diet_title'];?></option>                             
                              <?php
                            }
                            ?>
                          </select>
