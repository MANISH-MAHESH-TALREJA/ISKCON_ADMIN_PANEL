              ...
              <div class="section">
                <div class="section-body">
                  <div class="form-group">
                    ...
                    <div class="col-md-6">
                      <select class="select2" required="" name="slider_type">
                        <option value="video" <?=($row_data['slider_type']=='video') ? 'selected' : ''?>>Video</option>
                        <option value="diet" <?=($row_data['slider_type']=='diet') ? 'selected' : ''?>>Diet</option>
                  ...
                  <div class="form-group image_status" style="display: none;">
                    <label class="col-md-3 control-label" for="diet_id">Diets :-</label>
                    <div class="col-md-6">
                      <select name="diet_id" id="diet_id" class="select2">
                        <option value="">--Select Diets--</option>
                        <?php
                          $sql="SELECT * FROM tbl_diets WHERE tbl_diets.`status`=1  ORDER BY `id` DESC";
                          $res=mysqli_query($mysqli, $sql);
                          while($row=mysqli_fetch_array($res))
                          {
                        ?>                       
                          <option data-url="images/<?php echo $row['diet_image'];?>" value="<?php echo $row['id'];?>" <?=($row_data['post_id']==$row['id']) ? 'selected' : ''?>><?php echo $row['diet_title'];?></option>
                        <?php
                          }
                          mysqli_free_result($res);
                        ?>
...
<script type="text/javascript">
  ...
  var url=$("select[name='diet_id']").children("option:selected").data("url");
  $("select[name='diet_id']").parent("div").find(".preview").attr('src',url);
  $("select[name='diet_id']").parent("div").find(".preview").show();

  $("select[name='diet_id']").on("change",function(e){
    var url=$(this).children("option:selected").data("url");
    $(this).parent("div").find(".preview").attr('src',url);
    $(this).parent("div").find(".preview").show();
  });
