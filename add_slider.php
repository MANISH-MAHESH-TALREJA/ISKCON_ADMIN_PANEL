...
if(isset($_POST['submit']) and isset($_GET['add']))
{
  ...
  if($slider_type=='external'){
    ...
    switch ($slider_type) {
      ...
      $edit_status=Update('tbl_diets', $data_status, "WHERE id = ".$post_id);
      break;
      ...
      <form action="" name="addeditlanguage" method="post" class="form form-horizontal" enctype="multipart/form-data">
        ...
        <div class="section">
          <div class="section-body">
            ...
            <div class="form-group diet_status" style="display: none;">
              <label class="col-md-3 control-label" for="diet_id">Diet :-</label>
              <div class="col-md-6">
                <select name="diet_id" id="diet_id" class="select2">
                  <option value="">--Select Diet--</option>
                  <?php
                  $sql="SELECT * FROM tbl_diets WHERE tbl_diets.`status`=1 ORDER BY tbl_diets.`id` DESC";
                  $res=mysqli_query($mysqli, $sql);
                  while($row=mysqli_fetch_array($res))
                  {
                    ?>                       
                    <option data-url="images/<?php echo $row['diet_image'];?>" value="<?php echo $row['id'];?>"><?php echo $row['diet_title'];?></option>                           
                    <?php
                  }
                  mysqli_free_result($res);
                  ?>
...
<script type="text/javascript">
  $("select[name='slider_type']").on("change",function(e){
    ...
    $(".diet_status").find("select").attr("required",false);
    ...
    switch (type) {
      ...
      case 'diet':
      {
        ...
        $(".diet_status").show();
        $(".diet_status").find("select").attr("required",true);
        $(".video_status").hide();
        $(".external_banner").hide();
